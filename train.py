...
def process_type(file_name, seq_len):
    ...
    for line in file_data:
        ...
        a_list = [int(x) for x in data if x != "\n"]
        time_data.append(a_list)
    ...
    for line in time_data:
        sequences = []
        for end in range(seq_len, len(line) + 1):
            start = end - seq_len
    ...
    # Convert to torch tensor with dtype int64
    tensor = torch.tensor(np_data, dtype=torch.int64)
...
def generate_type(time_data):
    ...
    #print(len(type_data))
    #print(len(type_data[0]))
    #print(len(type_data[0][0]))
    return type_data
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_class", 
                        help="Number of types in the dataset, default is 2", type=int, default=75)
    ...
    file_name = '/content/RMTPP-pytorch/data/' + config.data + "/time-train.txt"
    time_train, time_duration = data_process(file_name, config.seq_len)
    if config.generate_type:
        type_data = generate_type(time_train)
    else:
        data_type = "/content/RMTPP-pytorch/data/"+config.data+"/event-train.txt"
        type_data = process_type(data_type, config.seq_len)
