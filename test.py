...
if __name__ == "__main__":
    ...
    data = "/content/RMTPP-pytorch/data/" + config.data + "/time-test.txt"

    time_test = data_process(data)
    if config.generate_type:
        type_test = generate_type(time_test)
    else:
        data_type = "/content/RMTPP-pytorch/data/"+config.data+"/event-test.txt"
    ...
    ax[0,0].plot(range(15),actual_duration)
    ax[0,0].plot(range(15),duration_pred)
    ax[0,1].plot(range(15),intensity_pred)
